...
class Login:
    ...
    def login(self, event=None):
        """Authenticate the user by checking the provided username and password with SQLite. """
        uname = self.username.get()
        pwd = self.password.get()
        self.cur.execute("INSERT OR IGNORE INTO users (username, password, account_type) VALUES ('Admin', 'Admin', 'ADMIN');")
    ...
    def register(self):
        """Create a new user account by registering the provided username and password in SQLite. """
        uname = self.username.get()
        pwd = self.password.get()
        ...
        if f:
            ...
            self.cur.execute("INSERT INTO users (username, password, account_type) VALUES (?, ?, 'USER')", (uname, pwd))
            self.con.commit()
