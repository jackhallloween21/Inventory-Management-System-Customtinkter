...
class Menu:
    ...
    def dashboard(self):
        ...
        x = 50
        self.cur.execute("SELECT COUNT(*) FROM orders WHERE date(date) = date('now');")
    ...
    def history(self):
        ...
        query = '''SELECT o.order_id, p.product_name, oi.quantity, oi.price, o.date, o.payment_status
                   FROM orders o
                   JOIN order_items oi ON o.order_id = oi.order_id
                   JOIN products p ON oi.product_id = p.product_id
                   WHERE o.customer = ?'''
        self.make_table(headings, 130)
        self.render_table(query=query, params=(self.user[0],))
    ...
    def add_item(self):
        ...
        new_win.resizable(False, False)  # Disable window resizing
        new_win.transient(self.window)  # Set the parent window
        new_win.grab_set()  # Make the window modal
    ...
    def remove_item(self):
        ...
        if selected_item and messagebox.askyesno('Alert!', 'Do you want to remove this item?'):
    ...
    def fill_labels(self, choice):
        ...
        self.cur.execute("SELECT quantity, price FROM products WHERE product_name = ?", (choice,))
    ...
    def add_to_cart(self):
        ...
        try:
            ...
            return
        self.cur.execute("SELECT product_id, description, price FROM products WHERE product_name = ?", (name,))
    ...
    def add_button(self):
        ...
        self.topwin.resizable(False, False)  # Disable window resizing
        self.topwin.transient(self.window)  # Set the parent window
        self.topwin.grab_set()  # Make the window modal
    ...
    def buy(self):
        ...
        row = self.cur.fetchone()
        order_item_id = row[0] + 1 if row else 1
        ...
        for item in all_items:
            ...
            quantity = values[-2]
            price = values[-1]
            self.cur.execute("INSERT INTO order_items (order_item_id, order_id, product_id, quantity, price) VALUES (?, ?, ?, ?, ?)",
                             (order_item_id, order_id, product_id, quantity, price))
            order_item_id += 1

        total_amount = sum(float(self.tree.item(item, "values")[-1]) for item in all_items)
        total_items = len(all_items)
        self.cur.execute("INSERT INTO orders (order_id, customer, date, total_items, total_amount, payment_status) VALUES (?, ?, ?, ?, ?, ?)",
                         (order_id, self.user[0], date.today(), total_items, total_amount, payment_status))
    ...
    def add_product(self):
        """Creates a new item in inventory by registering the provided details in SQLite. """
        ...
        if f:
            ...
            if len(p_desc) > 50:
                ...
                return
            self.cur.execute("INSERT INTO products (product_id, product_name, description, price, quantity) VALUES (?, ?, ?, ?, ?)",
