...
def add_graphs(cur, frame):
    ...
    try:
        ...
        order_status = [x[0] for x in payments]
        order_count = [x[1] for x in payments]

        figure = plt.Figure(figsize=(3, 3), dpi=100)
        ax = figure.add_subplot(1, 1, 1)

        ax.pie(order_count, labels=order_status, autopct="%1.1f%%", colors=colors[:len(order_status)], startangle=90)
    ...
    try:
        ...
        cur.execute("SELECT strftime('%m', o.date) AS month, SUM(oi.quantity * oi.price) AS earnings FROM orders o JOIN order_items oi ON o.order_id = oi.order_id WHERE o.payment_status = 'paid' AND strftime('%Y', o.date) = strftime('%Y', 'now') GROUP BY month ORDER BY month;")
        results = cur.fetchall()
        earnings = [0] * 12
        for month, earning in results:
            index = int(month) - 1  # Convert month to zero-based index
